...
const feriadosNacionais = [
  ...
  '2026-11-02', '2026-11-15', '2026-11-20', '2026-12-24', '2026-12-25', '2026-12-31',
  '2027-01-01', '2027-02-08', '2027-02-09', '2027-02-10', '2027-03-26', 
  '2027-04-21', '2027-05-01', '2027-05-27', '2027-09-07', '2027-10-12', 
  '2027-11-02', '2027-11-15', '2027-11-20', '2027-12-24', '2027-12-25', '2027-12-31',
...
export default function App() {
  ...
  function isHoje(dia: number) {
    const hoje = new Date();
    return hoje.getDate() === dia && hoje.getMonth() === mes && hoje.getFullYear() === ano;
  }

  function getFeriasDoDia(colabId: string, dia: number) {
    const dataVerificada = `${ano}-${String(mes + 1).padStart(2, '0')}-${String(dia).padStart(2, '0')}`;
    return ferias.find(f => {
      if (f.colaborador_id !== colabId) return false;
      return dataVerificada >= f.data_inicio && dataVerificada <= f.data_termino;
    });
  }

  function mudarMes(delta: number) { setDataAtual(new Date(ano, mes + delta, 1)); }
  function voltarParaHoje() { setDataAtual(new Date()); }

  function exportarExcel() {
    const feriasDoAno = ferias.filter(f => f.data_inicio.startsWith(ano.toString()) || f.data_termino.startsWith(ano.toString()));
  ...
  async function adicionarColaborador(e: React.FormEvent) {
    ...
    await supabase.from('colaboradores').insert([{ nome_completo: novoColaborador.trim(), equipe: novaEquipe }]);
    setNovoColaborador('');
  }
                ...
                {colaboradoresPorEquipe[equipe].map(colab => (
                  <tr key={colab.id}>
                    ...
                    {dias.map(dia => {
                      ...
                      const dataVerificada = `${ano}-${String(mes + 1).padStart(2, '0')}-${String(dia).padStart(2, '0')}`;
                      const isFimDeSemana = new Date(ano, mes, dia).getDay() === 0 || new Date(ano, mes, dia).getDay() === 6;
          ...
          <div className="modal-content">
            ...
            <form onSubmit={salvarFerias}>
              <div className="form-group">
                ...
                <select className="form-control" value={formData.colaborador_id} onChange={e => setFormData({...formData, colaborador_id: e.target.value})} required>
              ...
              <div style={{ display: 'flex', gap: '15px' }}>
                ...
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  <input type="date" className="form-control" value={formData.data_termino} onChange={e => setFormData({...formData, data_termino: e.target.value})} required />
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Observação (Opcional)</label>
                <input type="text" className="form-control" placeholder="Ex: Viagem marcada" value={formData.observacao || ''} onChange={e => setFormData({...formData, observacao: e.target.value})} />
              </div>
              <div className="modal-actions">
                {formData.id && <div className="actions-left"><button type="button" className="btn btn-danger" onClick={excluirFerias}><Trash2 size={16} /> Excluir</button></div>}
                ...
                <tbody>
                  {colaboradores.map(c => (
                    <tr key={c.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                      <td style={{ padding: '10px 15px', fontWeight: '500', color: '#334155' }}>
                        {c.nome_completo}
